    ...
    def delete_resource(self, resource, res):
        ...
        if handler:
            ...
            if resource == 'port':
                ...
                router_ifs_internal = self.conn.list_router_interfaces(res, 'internal')
                for port in router_ifs_internal:
                    self.log(f"Detaching interface {port.id} from router {res.id}")
                    try:
                        self.conn.network.remove_interface_from_router(
                            res.id, port_id=port.id)
                        self.log(f"Detached interface {port.id} from router {res.id}")
                    except Exception as e:
                        self.log(
                            f"Failed to detach interface {port.id} from router {res.id}: {str(e)}")
            delete_method(self.conn, res.id)
            self.log(f"{resource} with ID: {res.id} deleted successfully".capitalize())
        else:
            raise Exception(f"Unsupported resource type: {resource}")
